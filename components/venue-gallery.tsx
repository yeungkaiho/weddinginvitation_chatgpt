...
export function VenueGallery(){
 ...
 <div className="gallery-controls"><button type="button" aria-label="上一張場地照片" onClick={()=>api?.scrollPrev()}><ChevronLeft size={20}/></button><span>{selected+1} / {photos.length}</span><button type="button" aria-label="下一張場地照片" onClick={()=>api?.scrollNext()}><ChevronRight size={20}/></button>{!reduced&&<button type="button" aria-label={paused?'播放相片輪播':'暫停相片輪播'} onClick={()=>setPaused(v=>!v)}>{paused?<Play size={18}/>:<Pause size={18}/>}</button>}</div>
 </Carousel>;
}
